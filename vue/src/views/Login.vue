<template>
  <div class="container">
    <div style="width: 350px; padding: 50px 30px; background-color: rgba(0,0,0,.3); border-radius: 5px; box-shadow: 0 2px 10px rgba(0,0,0,.1)">
      <div style="text-align: center; font-size: 20px; margin-bottom: 20px; color: #046fdc"><b>HW外贸公司业务管理系统</b></div>
      <el-form :model="form" :rules="rules" ref="formRef">
        ...
        <el-form-item>
          <el-select style="width: 100%" v-model="form.role">
            <el-option v-for="item in [{ name: '管理员', value: 'ADMIN' }, { name: '员工', value: 'STAFF'  },{ name: '工厂', value: 'FACTORY'  }]" :key="item.id" :value="item.value" :label="item.name"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
        ...
        <div style="display: flex; align-items: center">
          ...
          <div style="flex: 1; text-align: right">
            <a style="color: #046fdc; font-weight: bold" href="/register">员工/工厂注册</a>
          </div>
        </div>
      </el-form>
    </div>
